...
void write_block_bitmap(int fd) {
    ...
    if (write(fd, bitmapNew, BLOCK_SIZE) != BLOCK_SIZE) {
        errno_exit("write");
    }
}
...
void write_inode_bitmap(int fd) {
    ...
    u32 used_inodes[] = {EXT2_ROOT_INO, LOST_AND_FOUND_INO, HELLO_WORLD_INO, HELLO_INO};
    for (size_t i = 0; i < sizeof(used_inodes) / sizeof(used_inodes[0]); i++) {
        u32 inode = used_inodes[i];
        bitmapNew[(inode - 1) / 8] |= (1 << ((inode - 1) % 8));
    }

    size_t lastByte = (NUM_INODES - 1) / 8;
    u8 unused_bits = (NUM_INODES - 1) % 8 + 1;
    bitmapNew[lastByte] |= (~0 << unused_bits);
...
void write_inode_table(int fd) {
    ...
    root_inode.i_mode = EXT2_S_IFDIR | EXT2_S_IRUSR | EXT2_S_IWUSR | EXT2_S_IXUSR |
...
void write_root_dir_block(int fd) {
    ...
    struct {
        ...
        {EXT2_ROOT_INO, "."},                 
        {EXT2_ROOT_INO, ".."},                
        {LOST_AND_FOUND_INO, "lost+found"},   
        {HELLO_WORLD_INO, "hello-world"},     
        {HELLO_INO, "hello"}                  
    };

    for (int i = 0; i < sizeof(entries) / sizeof(entries[0]); i++) {
        struct ext2_dir_entry entry = {0};
        dir_entry_set(entry, entries[i].inode, entries[i].name);
        dir_entry_write(entry, fd);
        byteLefts -= entry.rec_len;
    }

    if (byteLefts > 0) {
        struct ext2_dir_entry padding_entry = {0};
        padding_entry.rec_len = byteLefts;
        if (write(fd, &padding_entry, byteLefts) != byteLefts) {
            errno_exit("write");
        }
    }
}
...
void write_lost_and_found_dir_block(int fd) {
    if (lseek(fd, BLOCK_OFFSET(LOST_AND_FOUND_DIR_BLOCKNO), SEEK_SET) == -1) {
        errno_exit("lseek");
    }
    ssize_t byteLefts = BLOCK_SIZE;
