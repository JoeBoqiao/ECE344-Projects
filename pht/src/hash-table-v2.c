...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		pthread_mutex_init(&entry->mutexV2, NULL);
...
{
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	pthread_mutex_lock(&hash_table_entry->mutexV2);
	struct list_head *list_head = &hash_table_entry->list_head;
	struct list_entry *list_entry = get_list_entry(list_head, key);
	...
	pthread_mutex_unlock(&hash_table_entry->mutexV2);
}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		struct hash_table_entry *entry = &hash_table->entries[i];
		pthread_mutex_destroy(&entry->mutexV2);
